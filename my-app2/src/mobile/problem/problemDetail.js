...
import {Pagination, Icon, Modal, Avatar, Button, message, Input, Row, Col, List , Card, Divider,Comment,Form ,Empty, Collapse } from 'antd';
			...
			<div  style={{ marginTop:5}} className="backCD">
				{
					this.state.commentAll.length==0?<Empty description={"暂无数据"} />:this.state.commentAll.map(item => {
						return(
						<div>
							<CommentView username={item.ansBody} commentBody={item.ansBody} image={item.image} 
