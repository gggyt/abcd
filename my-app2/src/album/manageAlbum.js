...
import { Upload, Icon, Modal, Input, Pagination, Dropdown, Menu } from 'antd';
        ...
        <div>
          <Col span={6}>
              <Card
                ...
                cover={
                  <div>
                  <div className="iamgeD" onMouseOver = {this.mouseOn.bind(this)}
                onMouseOut = {this.mouseNot.bind(this)}>
                     <a href={'/home/albumDetail/'+this.props.albumId} target="_blank"><img 
                     className="imag" alt="example" src={this.props.coverPhotoId}  onClick = {this.onClick.bind(this)} />
                      </a>
                  </div>
                  </div>
                }
...
class AllAlbum extends React.Component{
  ...
  getClass() {
    fetch(SelectAlbum, {
      ...
      body:'pageNum='+this.state.nowPage
    }).then( res=> res.json()).then(
      data => {
        if (data.code==0) {
...
class ManageAlbum extends React.Component{
  ...
  render() {
    ...
    if (this.state.show==2) {
      sh = <Photo albumId={this.state.albumId} toManagePhoto1={this.toManagePhoto1.bind(this)}/>
    }
    return(
      <div className="bodyMain">
        <div  style={{width:1000, margin: '0 auto', padding: 10, backgroundColor:'#ffffff'}}>
          {sh}
        </div>
      </div>
    );
