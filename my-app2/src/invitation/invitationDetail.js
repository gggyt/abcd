...
class CommentView extends React.Component{
	...
	getComment() {
		...
		fetch(SelectComment,{   //Fetch方法
			...
			body: 'invitationId='+this.props.invitationId+'&pageNum=1'+'&pageSize='+this.state.pageSize+'&p_commentId='+this.props.commentId
	...
	render(){
		...
		if (this.state.totalPage>0) {
			pageView = <div className="floatR" style={{ marginBottom:30}}>
		...
		return(
			<div  style={{ marginTop:5}} className="backCD">
				...
				<div className="floatR">
     	 			<Pagination size="small" onChange={this.pageChange} total={this.state.totalPage*this.state.pageSize} 
     	 			defaultCurrent={this.state.nowPage} hideOnSinglePage={true} />
     	 		</div>
			</div>
		);
	}
...
class InvitationDetail extends React.Component{
    state={
     id:1
	}
